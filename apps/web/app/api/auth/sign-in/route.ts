...
function safeInvalidCredsMessage() {
...
export async function POST(request: Request) {
  ...
  const rate = requireRateLimit(
    request,
    { keyPrefix: "auth:sign-in", limit: 10, windowMs: 60_000 },
  ...
  if (!email || !email.includes("@") || password.length < 8) {
    return jsonError(
      { code: "VALIDATION_ERROR", message: "Please enter a valid email and password." },
      traceId,
  ...
  try {
    ...
    if (!res.ok) {
      return jsonError(
        { code: "AUTH_INVALID_CREDENTIALS", message: safeInvalidCredsMessage() },
        traceId,
        { status: 401, headers },
      );
    }
    ...
    return jsonError(
      { code: "INTERNAL", message: "Could not sign in. Please try again." },
      traceId,
